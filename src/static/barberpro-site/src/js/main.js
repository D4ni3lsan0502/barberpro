...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Calendar day selection (simplificado para um único barbeiro)
    ...
    calendarDays.forEach(day => {
        day.addEventListener("click", () => {
            ...
            const timeSlotHeader = document.querySelector("h5.text-md.font-medium.text-white.mb-3");
            if (timeSlotHeader) {
    ...
    timeSlots.forEach(slot => {
        slot.addEventListener("click", () => {
            ...
            if (slot.classList.contains("selected")) {
                slot.classList.add("bg-white", "text-black");
            } else {
                slot.classList.remove("bg-white", "text-black");
            }
        });
    });
    ...
    // Handle new client selection (simplificado, não há seleção de cliente)
    ...
    // Payment modal (simplificado, não há modal de pagamento separado)
    ...
    // Payment method selection (simplificado)
    ...
    // Client modal (simplificado, não há modal de cliente separado)
    ...
    // Add event listeners to client names in the table (simplificado)
    ...
    fab.className = "fab fixed bottom-6 right-6 bg-white text-black rounded-full w-14 h-14 flex items-center justify-center shadow-lg z-30 hover:bg-gray-200";
    fab.innerHTML = "<i class=\"fas fa-plus text-xl\"></i>";
    fab.addEventListener("click", window.openAppointmentModal);
    document.body.appendChild(fab);
    
    // Add event listeners to action buttons in the table (simplificado)
    const confirmButtons = document.querySelectorAll("td .text-white");
    if (confirmButtons.length > 0) {
        confirmButtons.forEach(button => {
            button.addEventListener("click", () => {
                // window.openPaymentModal(); // Não há modal de pagamento separado
                window.showNotification("Agendamento confirmado!"); // Notificação direta
            });
        });
    }
    ...
    // Copiar código PIX (simplificado)
