...
function atualizarEstatisticas(agendamentos) {
    ...
    let avaliacaoMedia = 5.0; // Valor padrão para Matheus Costa
    ...
    proximosAgendamentos.forEach(agendamento => {
        ...
        tr.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                ...
                <button class="text-red-600 hover:text-red-700 btn-cancelar" data-id="${agendamento.id}">
...
function exibirNotificacao(mensagem, tipo) {
    ...
    if (tipo === "success") {
        notificacao.classList.add("bg-green-600", "text-black");
    } else if (tipo === "error") {
        notificacao.classList.add("bg-red-600", "text-white");
    } else {
        notificacao.classList.add("bg-white", "text-black");
    }
...
function configurarEventListeners() {
    ...
    if (btnVerTodos) {
        btnVerTodos.addEventListener("click", function() {
            window.location.href = "agendamentos-barbeiro.html";
        });
    }
}
